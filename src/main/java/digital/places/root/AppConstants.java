package digital.places.root;

import java.util.LinkedHashMap;
import java.util.Map;

public class AppConstants
{
    public static final String PAGE403 = "403";
    public static final String PAGE403URL = "/403";

    public static final String LOGINPAGE = "login";
    public static final String LOGINPAGEURL = "/login";

    public static final String SEARCHPAGEURL ="/user/search";
    public static final String SEARCHPAGE ="search";


    public static Map<String,String> DD = new LinkedHashMap<String,String>();
    static
    {
	DD.put("","DD");
	DD.put("01","01");
	DD.put("02","02");
	DD.put("03","03");
	DD.put("04","04");
	DD.put("05","05");
	DD.put("06","06");
	DD.put("07","07");
	DD.put("08","08");
	DD.put("09","09");
	DD.put("10","10");
	DD.put("11","11");
	DD.put("12","12");
	DD.put("13","13");
	DD.put("14","14");
	DD.put("15","15");
	DD.put("16","16");
	DD.put("17","17");
	DD.put("18","18");
	DD.put("19","19");
	DD.put("20","20");
	DD.put("21","21");
	DD.put("22","22");
	DD.put("23","23");
	DD.put("24","24");
	DD.put("25","25");
	DD.put("26","26");
	DD.put("27","27");
	DD.put("28","28");
	DD.put("29","29");
	DD.put("30","30");
	DD.put("31","31");
    }

    public static Map<String,String> MM = new LinkedHashMap<String,String>();
    static
    {
	MM.put("","MM");
	MM.put("00","01");
	MM.put("01","02");
	MM.put("02","03");
	MM.put("03","04");
	MM.put("04","05");
	MM.put("05","06");
	MM.put("06","07");
	MM.put("07","08");
	MM.put("08","09");
	MM.put("09","10");
	MM.put("10","11");
	MM.put("11","12");
    }

    public static Map<String,String> YYYY = new LinkedHashMap<String,String>();
    static
    {
	YYYY.put("","YYYY");
	YYYY.put("1920","1920");
	YYYY.put("1921","1921");
	YYYY.put("1922","1922");
	YYYY.put("1923","1923");
	YYYY.put("1924","1924");
	YYYY.put("1925","1925");
	YYYY.put("1926","1926");
	YYYY.put("1927","1927");
	YYYY.put("1928","1928");
	YYYY.put("1929","1929");
	YYYY.put("1930","1930");
	YYYY.put("1931","1931");
	YYYY.put("1932","1932");
	YYYY.put("1933","1933");
	YYYY.put("1934","1934");
	YYYY.put("1935","1935");
	YYYY.put("1936","1936");
	YYYY.put("1937","1937");
	YYYY.put("1938","1938");
	YYYY.put("1939","1939");
	YYYY.put("1940","1940");
	YYYY.put("1941","1941");
	YYYY.put("1942","1942");
	YYYY.put("1943","1943");
	YYYY.put("1944","1944");
	YYYY.put("1945","1945");
	YYYY.put("1946","1946");
	YYYY.put("1947","1947");
	YYYY.put("1948","1948");
	YYYY.put("1949","1949");
	YYYY.put("1950","1950");
	YYYY.put("1951","1951");
	YYYY.put("1952","1952");
	YYYY.put("1953","1953");
	YYYY.put("1954","1954");
	YYYY.put("1955","1955");
	YYYY.put("1956","1956");
	YYYY.put("1957","1957");
	YYYY.put("1958","1958");
	YYYY.put("1959","1959");
	YYYY.put("1960","1960");
	YYYY.put("1961","1961");
	YYYY.put("1962","1962");
	YYYY.put("1963","1963");
	YYYY.put("1964","1964");
	YYYY.put("1965","1965");
	YYYY.put("1966","1966");
	YYYY.put("1967","1967");
	YYYY.put("1968","1968");
	YYYY.put("1969","1969");
	YYYY.put("1970","1970");
	YYYY.put("1971","1971");
	YYYY.put("1972","1972");
	YYYY.put("1973","1973");
	YYYY.put("1974","1974");
	YYYY.put("1975","1975");
	YYYY.put("1976","1976");
	YYYY.put("1977","1977");
	YYYY.put("1978","1978");
	YYYY.put("1979","1979");
	YYYY.put("1980","1980");
	YYYY.put("1981","1981");
	YYYY.put("1982","1982");
	YYYY.put("1983","1983");
	YYYY.put("1984","1984");
	YYYY.put("1985","1985");
	YYYY.put("1986","1986");
	YYYY.put("1987","1987");
	YYYY.put("1988","1988");
	YYYY.put("1989","1989");
	YYYY.put("1990","1990");
	YYYY.put("1991","1991");
	YYYY.put("1992","1992");
	YYYY.put("1993","1993");
	YYYY.put("1994","1994");
	YYYY.put("1995","1995");
	YYYY.put("1996","1996");
    }

}
