...
class UserRoleDAO extends AuthBase implements UserRoleFacade
{
	...
	public List<UserRole> findAll(String user)
	{
		...
		return (List<UserRole>) dataService
				.query("select ur from UserRole ur, Role r where r.enabled=1 and ur.enabled=1 and ur.roleid = r.roleid and ur.username = '"
						+ StringUtils.trimAllWhitespace(user) + "'");
	}
	...
	public int add(UserRole userRole) throws Exception
	{
		...
		{
			// For add page - readonly view of existing records in case of
			// errors
	...
	private void validateUserRole(UserRole userRole) throws Exception
	{
		try
		{
			userRole.setRolestartdate(
					parseDate(userRole.getRolesddd(), userRole.getRolesdmm(), userRole.getRolesdyyyy()));
			if (!StringUtils.isEmpty(userRole.getRolesddd()))
			{
				userRole.setRoleenddate(
						parseDate(userRole.getRolesddd(), userRole.getRolesdmm(), userRole.getRolesdyyyy()));
			}
		}
		catch (Exception e)
